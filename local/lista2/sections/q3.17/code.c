...
void Read_vector(
   ...
   MPI_Type_contiguous(local_n, MPI_DOUBLE, &CONTIGUOUS);
   ...
   if (my_rank == 0) {
      ...
      MPI_Scatter(a, 1, CONTIGUOUS, local_a, 1, CONTIGUOUS, 0, comm);
      free(a);
   } else {
      Check_for_error(local_ok, fname, "Can't allocate temporary vector",
            comm);
...
void Print_vector(
   ...
   if (my_rank == 0) {
      ...
      Check_for_error(local_ok, fname, "Can't allocate temporary vector",
            comm);
      MPI_Gather(local_b, 1, CONTIGUOUS, b, 1, CONTIGUOUS, 0, comm);
      printf("%s\n", title);
      for (i = 0; i < n; i++)
         printf("%f ", b[i]);
      printf("\n");
